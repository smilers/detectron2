...
def densepose_chart_predictor_output_hflip(
    ...
    for field in fields(densepose_predictor_output):
        ...
        if isinstance(field_value, torch.Tensor):
            setattr(densepose_predictor_output, field.name,
                    torch.flip(field_value, [3]))
...
def _flip_iuv_semantics_tensor(
    ...
    u_loc = (densepose_predictor_output.u[:, 1:, :, :].clamp(0, 1) *
             255).long()
    v_loc = (densepose_predictor_output.v[:, 1:, :, :].clamp(0, 1) *
             255).long()
    Iindex = torch.arange(
        C - 1, device=densepose_predictor_output.u.device)[None, :, None,
                                                           None].expand(
                                                               N, C - 1, H, W)
    ...
    for el in ["fine_segm", "u", "v"]:
        densepose_predictor_output.__dict__[
            el] = densepose_predictor_output.__dict__[
                el][:, point_label_symmetries, :, :]
    return densepose_predictor_output
