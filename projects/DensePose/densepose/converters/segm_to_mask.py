...
def resample_coarse_segm_tensor_to_bbox(coarse_segm: torch.Tensor,
    ...
    w = max(int(w), 1)
    h = max(int(h), 1)
    return F.interpolate(coarse_segm, (h, w),
                         mode="bilinear",
                         align_corners=False).argmax(dim=1)


def resample_fine_and_coarse_segm_tensors_to_bbox(fine_segm: torch.Tensor,
    ...
    h = max(int(h), 1)
    # coarse segmentation
    coarse_segm_bbox = F.interpolate(coarse_segm, (h, w),
                                     mode="bilinear",
                                     align_corners=False).argmax(dim=1)
    return (F.interpolate(
        fine_segm,
        (h, w), mode="bilinear", align_corners=False).argmax(dim=1) *
            (coarse_segm_bbox > 0).long())


def resample_fine_and_coarse_segm_to_bbox(predictor_output: Any,
                                          box_xywh_abs: IntTupleBox):
    """
